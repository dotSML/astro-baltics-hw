...
import { parseDate } from 'src/core/utils/date.utils';
...
export class EstonianPersonalCodeStrategy extends PersonalCodeStrategy {
  public validate(personalCode: string): {
    ...
    const { year, month, day } = this.extractDateFromPersonalCode(personalCode);
  ...
  public generate(gender: Gender, dateOfBirth: string) {
  ...
  private extractDateFromPersonalCode(personalCode: string): {
    ...
    const year =
      ...
      parseInt(personalCode.substr(1, 2), 10);
    const month = parseInt(personalCode.substr(3, 2), 10);
    const day = parseInt(personalCode.substr(5, 2), 10);
  ...
  private getBirthCentury(genderCode: number) {
  ...
  private calculateControlNumber(digits: number[]) {
